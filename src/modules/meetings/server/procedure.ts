...
export const meetingsRouter = createTRPCRouter({
    generateChatToken: protectedProcedure.mutation(async({ctx})=>{
        ...
        streamChat.upsertUser({id:ctx.auth.user.id, role:"admin"})
        return token;
    }),
...
return transcriptWithSpeakers;
    ...
    getMany: protectedProcedure.input(z.object({
        ...
        status: z.enum([MeetingStatus.upcomming, MeetingStatus.active, MeetingStatus.completed, MeetingStatus.processing, MeetingStatus.cancelled]).nullish()
    })).query(async ({ ctx, input }) => {

        const { search, page, pageSize, agentId, status } = input;
        const data = await db.select({
            ...getTableColumns(meetings),
            agent: agents, duration: sql<number>`EXTRACT(EPOCH FROM (ended_at - started_at))`.as("duration")
